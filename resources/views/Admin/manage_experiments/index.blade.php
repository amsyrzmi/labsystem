<x-admin-layout>
    ...
    <div class="container no-shadow">
        ...
        <div class="page-header">
            ...
            <a href="{{ route('admin.manage_experiments.create') }}" class="btn btn-approve">
                + Create New Experiment
            </a>
        </div>

        <div class="filter-bar">
            <form method="GET" action="{{ route('admin.manage_experiments.index') }}" id="filterForm" style="width: 100%; display: flex; flex-wrap: wrap; gap: 16px; align-items: flex-end;">
                ...
                <div class="view-controls">
                    ...
                    @if(request()->hasAny(['form_level', 'subject_id', 'search']))
                        <a href="{{ route('admin.manage_experiments.index') }}" class="btn btn-cancel" style="margin-top:0; text-decoration: none;">Reset</a>
                    @endif
                </div>
            </form>
        </div>
        ...
        @forelse($experiments as $experiment)
            <div class="request-card">
                ...
                <div class="action-buttons">
                    <a href="{{ route('admin.manage_experiments.edit', $experiment->id) }}" class="btn btn-view">
                        Edit Details
                    </a>
                    
                    <form action="{{ route('admin.manage_experiments.destroy', $experiment->id) }}" method="POST" style="display:inline;">
                        @csrf @method('DELETE')
    ...
    <script>
        ...
        formFilter.addEventListener('change', async () => {
            ...
            try {
                const res = await fetch(`{{ url('/admin/api/subjects') }}/${formFilter.value}`);
