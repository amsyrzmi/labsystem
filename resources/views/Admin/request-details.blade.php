<x-admin-layout>
    ...
    <div class="container no-shadow" style="max-width:960px;margin:10px auto 40px;padding:18px;background:transparent;">
       ...
       <h1 style="font-size:26px;color:var(--accent);margin-bottom:6px;">Request #{{ $request->id }}</h1>
        ...
        <!-- After all the info cards, add Admin Actions -->
        @if(in_array($request->status, ['pending', 'approved']))
            <div style="background:white;border-radius:12px;padding:24px;margin-top:24px;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
                <h3 style="color:#1b263b;margin-bottom:16px;">Admin Actions</h3>
                
                <div style="display:flex;gap:12px;flex-wrap:wrap;">
                    @if($request->status === 'approved')
                        <form action="{{ route('admin.requests.complete', $request->id) }}" method="POST" style="display:inline;">
                            @csrf
                            <button type="submit" onclick="return confirm('Mark this request as completed?')" 
                                    style="padding:12px 24px;background:#28a745;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;">
                                ✓ Mark as Completed
                            </button>
                        </form>
                    ...
                    <form action="{{ route('admin.requests.delete', $request->id) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" onclick="return confirm('Delete this request permanently? This cannot be undone.')" 
                                style="padding:12px 24px;background:#6c757d;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;">
                            Delete Request
                        </button>
                    </form>
                </div>
