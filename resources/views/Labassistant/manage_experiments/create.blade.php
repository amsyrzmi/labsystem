<x-lab-assistant-layout>
    ...
    <div class="container no-shadow">
        ...
        <form method="POST" action="{{ route('lab_assistant.manage_experiments.store') }}">
            @csrf
            ...
            <div style="display: flex; justify-content: flex-end; gap: 12px; padding-bottom: 40px;">
                <a href="{{ route('lab_assistant.manage_experiments.index') }}" class="btn btn-cancel" style="display: inline-flex; align-items: center;">
                    Discard
                </a>
    ...
    <script>
        ...
        document.getElementById('formLevel').addEventListener('change', async function() {
            ...
            if (this.value) {
                try {
                    const res = await fetch(`{{ url('/lab-assistant/api/subjects') }}/${this.value}`);
        ...
        subjectSelect.addEventListener('change', async function() {
            ...
            if (this.value) {
                try {
                    const res = await fetch(`{{ url('/lab-assistant/api/topics') }}/${this.value}`);
                    const data = await res.json();
