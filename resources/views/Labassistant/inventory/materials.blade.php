<x-lab-assistant-layout>
                            ...
                            @foreach($materials as $material)
                                <tr>
                                    <td>
                                        ...
                                        @if($material->notes)
                                            <br><small style="color: #7b8aa3;">{{ Str::limit($material->notes, 50) }}</small>
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        @if($material->quantity == 0)
                                            <span class="stock-indicator stock-critical">Out of Stock</span>
                                        @elseif($material->quantity <= $material->minimum_quantity)
                                            <span class="stock-indicator stock-low">Low Stock</span>
                                        @else
                                            <span class="stock-indicator stock-ok">In Stock</span>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="action-buttons">
                                            <button onclick="openEditModal({{ $material->id }})" class="btn-icon btn-edit">
                                                ✏️ Edit
                                            </button>
                                            <button onclick="confirmDelete({{ $material->id }})" class="btn-icon btn-delete">
                                                🗑️ Delete
                                            </button>
                                        </div>
                                    </td>
    ...
    <div id="addModal" class="modal">
        <div class="modal-content">
            ...
            <form action="{{ route('lab_assistant.inventory.materials.store') }}" method="POST">
                @csrf
                ...
                <div class="form-group">
                    <label class="form-label">Concentration (mol/dm³)</label>
                    <input type="number" name="concentration" class="form-control" step="0.0001" min="0">
                    <div class="form-helper">Optional - for solutions only</div>
                </div>

                <div class="form-group">
                    <label class="form-label">Minimum Quantity (Alert Threshold) *</label>
                    <input type="number" name="minimum_quantity" class="form-control" step="0.01" min="0" required>
                    <div class="form-helper">You'll be alerted when stock falls below this level</div>
                </div>
    ...
    <div id="editModal" class="modal">
        <div class="modal-content">
            ...
            <form id="editForm" method="POST">
                ...
                <div class="form-group">
                    <label class="form-label">Concentration (mol/dm³)</label>
                    <input type="number" name="concentration" id="edit_concentration" class="form-control" step="0.0001" min="0">
                </div>
...
<script>
    // Materials data for editing
    const materials = @json($materials->items());

    // Base URLs for routes (without parameters)
    const updateRouteBase = "{{ route('lab_assistant.inventory.materials') }}";
    const deleteRouteBase = "{{ route('lab_assistant.inventory.materials') }}";
    ...
    function openEditModal(id) {
        ...
        document.getElementById('edit_concentration').value = material.concentration || '';
        document.getElementById('edit_minimum_quantity').value = material.minimum_quantity;
